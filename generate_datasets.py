...
def generate_shape_detection_dataset(output_dir, num_train=1000, num_val=200):
    ...
    def create_shape_image(image_id, num_objects, split='train'):
        ...
        for i in range(len(annotations)):
            ...
            # Draw shape
            if info['cls_name'] == 'circle':
                mask_draw.ellipse(bbox, fill=255)
            elif info['cls_name'] == 'square':
                mask_draw.rectangle(bbox, fill=255)
            elif info['cls_name'] == 'triangle':
                x, y, size = info['x'], info['y'], info['size']
                points = [
                    (x, y - size//2),
                    (x - size//2, y + size//2),
                    (x + size//2, y + size//2)
            ...
            # Total pixels
            mask_array = np.array(mask.crop(bbox))
            total_pixels = np.sum(mask_array > 0)
            
            # Occlusion mask
            occlusion = Image.new('L', (img_size, img_size), 0)
            occ_draw = ImageDraw.Draw(occlusion)
            
            # Draw later shapes
            for j in range(i+1, len(shape_info)):
                later = shape_info[j]
                later_bbox = later['bbox']
                
                if later['cls_name'] == 'circle':
                    occ_draw.ellipse(later_bbox, fill=255)
                elif later['cls_name'] == 'square':
                    occ_draw.rectangle(later_bbox, fill=255)
                elif later['cls_name'] == 'triangle':
                    x, y, size = later['x'], later['y'], later['size']
                    points = [
                        (x, y - size//2),
                        (x - size//2, y + size//2),
                        (x + size//2, y + size//2)
                    ]
                    occ_draw.polygon(points, fill=255)
            
            # Visible pixels
            mask_crop = np.array(mask.crop(bbox))
            occ_crop = np.array(occlusion.crop(bbox))
            visible_pixels = np.sum((mask_crop > 0) & (occ_crop == 0))
            
            # Add visibility
            visibility = visible_pixels / total_pixels if total_pixels > 0 else 0.0
            annotations[i]['visibility'] = float(visibility)
